...
const addLanguageToHeader = (requestHandler, getLangCode) => (
  type,
  resource,
  params,
  header,
...
export function* handleFetch(dataProvider, action) {
  const {
    type,
    payload,
    meta: { fetch: fetchMeta, onSuccess, onFailure, resolve, reject, ...meta },
  } = action;
  ...
  try {
    ...
    console.log(
      `restType ${restType} meta.resource ${meta.resource} payload`,
      payload,
      ' dataProvider ',
      dataProvider,
    );
